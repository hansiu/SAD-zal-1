...
wczytaj_i_przygotuj <- function(nazwa_pliku='pan-tadeusz.txt'){
  ...
  #plik <- iconv(plik,from="UTF-8",to="ASCII//TRANSLIT") #bez polskich radzi sobie ciut lepiej
  #plik <- gsub("a","ae",plik,fixed=TRUE) #poniewaz R nie widzi miedzy nimi roznicy, i python tez... tu zamienilam je w notepadzie..
  #plik <- gsub("a","a",plik)
  #plik <- gsub("é","e",plik)
  return(plik)
...
testuj <- function(){
  #l <- litery(wczytaj_i_przygotuj()[(1:20)])
  s <- slowa(wczytaj_i_przygotuj()[(1:20)])
  # Error in if (d < delta) { : missing value where TRUE/FALSE needed daje slowa?
  #return(c("litery"=l,"s³owa"=s))
  return(s)
}
