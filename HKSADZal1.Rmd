...
cat(names(grupy[grupy<0]))
lit_p <- data.frame(colnames(lit_polskie$hmm$emissionProbs),lit_polskie$hmm$emissionProbs[1,],lit_polskie$hmm$emissionProbs[2,])
rownames(lit_p) <- NULL
names(lit_p) <- c('names','probs1','probs2')
ggplot(lit_p, aes(names,probs1, fill=as.factor(names))) + geom_bar(stat="identity")
ggplot(lit_p, aes(names,probs2, fill=as.factor(names))) + geom_bar(stat="identity")
```
